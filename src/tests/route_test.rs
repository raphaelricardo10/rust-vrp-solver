...
fn route_distance_calculation(stops: Vec<Stop>, distances: DistanceMatrix) {
    ...
    let mut route = Route::new(&mut vehicle);

    route.add_stop(&stops[0], 0.0).unwrap();
    route.add_stop(&stops[1], *distances.get(&(stops[0].get_id(), stops[1].get_id())).unwrap()).unwrap();
    route.add_stop(&stops[2], *distances.get(&(stops[1].get_id(), stops[2].get_id())).unwrap()).unwrap();
    route.add_stop(&stops[3], *distances.get(&(stops[2].get_id(), stops[3].get_id())).unwrap()).unwrap();
...
fn route_cannot_overload_vehicle(full_stops: Vec<Stop>, distances: DistanceMatrix) {
    ...
    let distance = *distances.get(&(full_stops[0].get_id(), full_stops[1].get_id())).unwrap();
